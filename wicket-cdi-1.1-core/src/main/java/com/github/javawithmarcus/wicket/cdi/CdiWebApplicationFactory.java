 ...
 * enableApplicationInjection init-param.
 * <p/>
 * This class is also used by CdiWicketFilter. When using CdiWicketFilter you do not need to use the
 * applicationClassName init-param. CdiWicketFilter uses the Injected CdiWebApplicationFactory. When createApplication
 * is called a WebApplication Instance iterator is searched to find the proper WebApplication. Search works by first
 * looking up the init-param applicationName. If this is found then the WebApplication annotated with {@link WicketApp}
 * ('someApp') is selected. This allows for multiple WebApplications to exist in the classloader. If multiple
 * WebApplication are in the classloader then the use of {@link WicketApp} is required. If the init-param
 * applicationName is not set then the classloader is searched to verify there is only one WebApplication. If there is
 * only one WebApplication then it is used. If there are no WebApplications or multiple Applications without
 * applicationName being set then an exception will be thrown. When using CdiWicketFilter the Application is always
 * Injected. When using this in a EE container like glassfish the {@link WicketApp} is required to let EE container
 * manage the object.
 * 
 * @author jsarman
 */
        ...
        if (applications == null) {
            ...
            if (appName != null) {
                try {
                    ...
                    log.error("The init param {} set to {} requires a WebApplication to be annotated with @Named(\"{}\").", new Object[] {
                            WICKET_APP_NAME, appName, appName });
                    throw ure;
                }
            } else {
    ...
    private ConfigurationParameters buildParameters(FilterConfig filterConfig) {
        ...
        if (propagation != null) {
            try {
                ...
                log.warn("Init Param {} = {} is not a valid propagation type. Using Default {}", new Object[] { PROPAGATION, propagation,
